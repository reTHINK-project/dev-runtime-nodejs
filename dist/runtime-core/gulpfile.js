...
gulp.task('api', ['doc'], function () {
  ...
  return gulp.src('docs/jsdoc/*.html').pipe(pandoc({
    from: 'html',
    to: 'markdown_github',
    ext: '.md',
    args: ['--smart']
...
gulp.task('dist', function () {
  ...
  return gulp.src(['src/sandbox.js', 'src/minibus.js', 'src/runtime/RuntimeUA.js', 'src/policy/PolicyEngine.js', 'src/policy/context/MessageNodeCtx.js']).pipe(dist(debug)).on('end', function () {
...
function transpile(opts) {
  ...
  return through.obj(function (file, enc, cb) {
    ...
    return browserify(args).transform(babel, {
      ...
      this.emit('end');
    }).pipe(source(filename)).pipe(buffer()).pipe(gulpif(!opts.debug, sourcemaps.init({ loadMaps: true }))).pipe(gulpif(!opts.debug, uglify())).pipe(gulpif(!opts.debug, sourcemaps.write('./'))).pipe(gulp.dest(opts.destination)).on('end', function () {
