...
!function () {
  var t = function t(e) {
  ...
  };t.version = "0.5.11", t.utils = {}, t.utils.warn = function (t) {
    return function (e) {
    ...
    var n = new t.Pipeline();return e.forEach(function (e) {
      var i = t.Pipeline.registeredFunctions[e];if (!i) throw new Error("Cannot load un-registered function: " + e);n.add(i);
    }), n;
    ...
    t.Pipeline.warnIfFunctionNotRegistered(n);var i = this._stack.indexOf(e);if (-1 == i) throw new Error("Cannot find existingFn");i += 1, this._stack.splice(i, 0, n);
  }, t.Pipeline.prototype.before = function (e, n) {
    t.Pipeline.warnIfFunctionNotRegistered(n);var i = this._stack.indexOf(e);if (-1 == i) throw new Error("Cannot find existingFn");this._stack.splice(i, 0, n);
    ...
    for (var e = [], n = t.length, i = this._stack.length, o = 0; n > o; o++) {
      for (var r = t[o], s = 0; i > s && (r = this._stack[s](r, o, t), void 0 !== r); s++) {}void 0 !== r && e.push(r);
    }return e;
    ...
    this._magnitude = void 0;var i = this.list;if (!i) return this.list = new t.Vector.Node(e, n, i), this.length++;if (e < i.idx) return this.list = new t.Vector.Node(e, n, i), this.length++;for (var o = i, r = i.next; void 0 != r;) {
      if (e < r.idx) return o.next = new t.Vector.Node(e, n, r), this.length++;o = r, r = r.next;
    }return o.next = new t.Vector.Node(e, n, r), this.length++;
  }, t.Vector.prototype.magnitude = function () {
    if (this._magnitude) return this._magnitude;for (var t, e = this.list, n = 0; e;) {
    ...
    for (var e = 0, n = this.elements.length, i = n - e, o = e + Math.floor(i / 2), r = this.elements[o]; i > 1;) {
      if (r === t) return o;t > r && (e = o), r > t && (n = o), i = n - e, o = e + Math.floor(i / 2), r = this.elements[o];
    }return r === t ? o : -1;
    ...
    for (var n = new t.SortedSet(), i = 0, o = 0, r = this.length, s = e.length, a = this.elements, h = e.elements;;) {
      if (i > r - 1 || o > s - 1) break;a[i] !== h[o] ? a[i] < h[o] ? i++ : a[i] > h[o] && o++ : (n.add(a[i]), i++, o++);
    }return n;
    ...
    var e, n, i;return this.length >= t.length ? (e = this, n = t) : (e = t, n = this), i = e.clone(), i.add.apply(i, n.toArray()), i;
    ...
    e.version !== t.version && t.utils.warn("version mismatch: current " + t.version + " importing " + e.version);var n = new this();return n._fields = e.fields, n._ref = e.ref, n.documentStore = t.Store.load(e.documentStore), n.tokenStore = t.TokenStore.load(e.tokenStore), n.corpusTokens = t.SortedSet.load(e.corpusTokens), n.pipeline = t.Pipeline.load(e.pipeline), n;
    ...
    var i = {},
        o = new t.SortedSet(),
        r = e[this._ref],
        n = void 0 === n ? !0 : n;this._fields.forEach(function (n) {
      var r = this.pipeline.run(t.tokenizer(e[n.name]));i[n.name] = r, t.SortedSet.prototype.add.apply(o, r);
    }, this), this.documentStore.set(r, o), t.SortedSet.prototype.add.apply(this.corpusTokens, o.toArray());for (var s = 0; s < o.length; s++) {
      var a = o.elements[s],
          h = this._fields.reduce(function (t, e) {
        var n = i[e.name].length;if (!n) return t;var o = i[e.name].filter(function (t) {
          return t === a;
        }).length;return t + o / n * e.boost;
      }, 0);this.tokenStore.add(a, { ref: r, tf: h });
    ...
    var e = "@" + t;if (Object.prototype.hasOwnProperty.call(this._idfCache, e)) return this._idfCache[e];var n = this.tokenStore.count(t),
        i = 1;return n > 0 && (i = 1 + Math.log(this.documentStore.length / n)), this._idfCache[e] = i;
    ...
    var n = this.pipeline.run(t.tokenizer(e)),
    ...
    }, this);if (!s) return [];n.forEach(function (e, n, s) {
      var a = 1 / s.length * this._fields.length * r,
        ...
        var r = h.corpusTokens.indexOf(o),
            s = h.idf(o),
            l = 1,
            u = new t.SortedSet();if (o !== e) {
          var c = Math.max(3, o.length - e.length);l = 1 / Math.log(c);
        }return r > -1 && i.insert(r, a * s * l), Object.keys(h.tokenStore.get(o)).forEach(function (t) {
          u.add(t);
        }), n.union(u);
      }, new t.SortedSet());o.push(l);
    }, this);var a = o.reduce(function (t, e) {
    ...
    for (var n = this.documentStore.get(e), i = n.length, o = new t.Vector(), r = 0; i > r; r++) {
      var s = n.elements[r],
          ...
          h = this.idf(s);o.insert(this.corpusTokens.indexOf(s), a * h);
    }return o;
  }, t.Index.prototype.toJSON = function () {
    return { version: t.version, fields: this._fields, ref: this._ref, documentStore: this.documentStore.toJSON(), tokenStore: this.tokenStore.toJSON(), corpusTokens: this.corpusTokens.toJSON(), pipeline: this.pipeline.toJSON() };
    ...
    var n = new this();return n.length = e.length, n.store = Object.keys(e.store).reduce(function (n, i) {
    ...
    var t = { ational: "ate", tional: "tion", enci: "ence", anci: "ance", izer: "ize", bli: "ble", alli: "al", entli: "ent", eli: "e", ousli: "ous", ization: "ize", ation: "ate", ator: "ate", alism: "al", iveness: "ive", fulness: "ful", ousness: "ous", aliti: "al", iviti: "ive", biliti: "ble", logi: "log" },
        ...
        T = function T(n) {
      var i, o, r, s, a, h, l;if (n.length < 3) return n;if (r = n.substr(0, 1), "y" == r && (n = r.toUpperCase() + n.substr(1)), s = f, a = m, s.test(n) ? n = n.replace(s, "$1$2") : a.test(n) && (n = n.replace(a, "$1$2")), s = v, a = g, s.test(n)) {
        var T = s.exec(n);s = u, s.test(T[1]) && (s = y, n = n.replace(s, ""));
      } else if (a.test(n)) {
        var T = a.exec(n);i = T[1], a = d, a.test(i) && (n = i, a = S, h = w, l = x, a.test(n) ? n += "e" : h.test(n) ? (s = y, n = n.replace(s, "")) : l.test(n) && (n += "e"));
      }if (s = k, s.test(n)) {
        var T = s.exec(n);i = T[1], n = i + "i";
      }if (s = E, s.test(n)) {
        var T = s.exec(n);i = T[1], o = T[2], s = u, s.test(i) && (n = i + t[o]);
      }if (s = b, s.test(n)) {
        var T = s.exec(n);i = T[1], o = T[2], s = u, s.test(i) && (n = i + e[o]);
      }if (s = _, a = F, s.test(n)) {
        var T = s.exec(n);i = T[1], s = c, s.test(i) && (n = i);
      } else if (a.test(n)) {
        var T = a.exec(n);i = T[1] + T[2], a = c, a.test(i) && (n = i);
      }if (s = O, s.test(n)) {
        var T = s.exec(n);i = T[1], s = c, a = p, h = N, (s.test(i) || a.test(i) && !h.test(i)) && (n = i);
    ...
    var n = n || this.root,
        i = t[0],
        o = t.slice(1);return i in n || (n[i] = { docs: {} }), 0 === o.length ? (n[i].docs[e.ref] = e, void (this.length += 1)) : this.add(o, e, n[i]);
  }, t.TokenStore.prototype.has = function (t) {
    if (!t) return !1;for (var e = this.root, n = 0; n < t.length; n++) {
      if (!e[t[n]]) return !1;e = e[t[n]];
    }return !0;
  }, t.TokenStore.prototype.getNode = function (t) {
    if (!t) return {};for (var e = this.root, n = 0; n < t.length; n++) {
    ...
    if (t) {
      for (var n = this.root, i = 0; i < t.length; i++) {
        if (!(t[i] in n)) return;n = n[t[i]];
      }delete n.docs[e];
    }
  }, t.TokenStore.prototype.expand = function (t, e) {
    var n = this.getNode(t),
        i = n.docs || {},
        e = e || [];return Object.keys(i).length && e.push(t), Object.keys(n).forEach(function (n) {
