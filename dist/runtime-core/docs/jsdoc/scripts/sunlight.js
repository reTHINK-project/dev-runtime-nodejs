...
(function (window, document, undefined) {
	...
	function _getNextWhile(tokens, index, direction, matcher) {
		var count = 1,
		    token;
	...
	window.Sunlight = {
		...
		registerLanguage: function registerLanguage(languageId, languageData) {
			...
			for (tokenName in languageData.customTokens) {
				languageData.customTokens[tokenName] = createHashMap(languageData.customTokens[tokenName].values, languageData.customTokens[tokenName].boundary, languageData.caseInsensitive);
			}
		...
		util: {
			...
			getNextNonWsToken: function getNextNonWsToken(tokens, index) {
				return _getNextWhile(tokens, index, 1, function (token) {
					return token.name === "default";
				});
			},
			getPreviousNonWsToken: function getPreviousNonWsToken(tokens, index) {
				return _getNextWhile(tokens, index, -1, function (token) {
					return token.name === "default";
				});
			},
			getNextWhile: function getNextWhile(tokens, index, matcher) {
				return _getNextWhile(tokens, index, 1, matcher);
			},
			getPreviousWhile: function getPreviousWhile(tokens, index, matcher) {
				return _getNextWhile(tokens, index, -1, matcher);
			},
