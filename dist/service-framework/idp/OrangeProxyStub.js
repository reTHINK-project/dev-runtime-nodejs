  ...
  generateAssertion: function generateAssertion(contents /*, origin, hint */) {
    ...
    return new Promise(function (resolve, reject) {
      return getProxyID().then(function (ID) {
        var _url = SOURCEURL + AUTHPATH + '?scope=' + FULLSCOPE + '&client_id=' + ID + '&redirect_uri=' + SOURCEURL + DONEPATH + '&response_type=' + TYPE + '&nonce=' + 'N-' + Math.random() + '&rtcsdp=' + btoa(contents);
        ...
        return crypto.subtle.importKey('jwk', Key, { name: 'RSASSA-PKCS1-v1_5', hash: { name: "SHA-256" } }, true, ['verify']).then(function (JWK) {
          return (
            ...
            .then(function (result) {
              if (!result) reject(new Error('Invalid signature on identity assertion'));else {
