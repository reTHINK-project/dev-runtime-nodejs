  ...
  generateAssertion: function generateAssertion(contents, origin, hint) {
    ...
    return new Promise(function (resolve, reject) {
      if (!contents) {
        ...
        var requestUrl = m.tokenEndpoint + 'response_type=' + m.type + '&client_id=' + m.clientID + '&scope=' + m.scope + '&nonce=' + m.nonce + '&response_mode=' + m.mode + '&redirect_uri=' + m.redirectURI;

        reject({ name: 'IdPLoginError', loginUrl: requestUrl });
      } else {

        //later verify the token and use the information from the JWT
        var contentSplited = contents.split('.');

        var idToken = JSON.parse(atob(contentSplited[1]));

        var idpBundle = { domain: 'google.com', protocol: 'OIDC' };
        var identityBundle = { assertion: contentSplited[1], idp: idpBundle, infoToken: idToken };
